...
//System.out.println("After adding player to new room: " + board.getPlayerLocations());
 ...
 /* *******************************************************/
    ...
    private boolean isFourConnectedMove(BotManager bot, int row, int col) {
        if (row < 0 || row >= 3) return false;
        if (col < 0 || col >= 4) return false;
        if (Math.abs(bot.col - col) + Math.abs(bot.row - row) != 1)
            return false;
        return true;
    }
    ...
    private boolean checkDiceAction2Moves(Dice dice, String action) {
        ...
        if (!isFourConnectedTwoMoves(bot, row, col))
            throw new ActionException("Bad move for " + guestName + ". Move from " + bot.row + "," + bot.col + " to " + row + "," + col);
        return true;
    }
    ...
    private boolean checkDiceAction(Dice dice, String action) {
        ...
        int row = Integer.parseInt(split[2].trim());
        int col = Integer.parseInt(split[3].trim());
        if (!isFourConnectedMove(bot, row, col))
            throw new ActionException("Bad move for " + guestName + ". Move from " + bot.row + "," + bot.col + " to " + row + "," + col);
        return true;
    }
    ...
    private boolean checkCardAction(BotManager player, String action, String cardAction) {
        ...
        if (action.startsWith("move")) {
            ...
            int col = Integer.parseInt(split[3].trim());
            if (row < 0 || row >= 3)
                throw new ActionException("Bad move action from card, row value out of range.");
            if (col < 0 || col >= 4)
                throw new ActionException("Bad move action from card, col value out of range.");
            return true;
            ...
            if (!playerNamesVector.contains(action.split(",")[2]))
                throw new ActionException("Bad player name " + action.split(",")[2] + " in ask action.");
            return true;
        } else return false;
    }
